---
title: "README (för lärare)" 
description: Kursdokumentation för lärare.
revision:
  2025-06-26: (B, mos) Nu både mirror och fork med exempelrepon. 
  2025-06-24: (A, mos) Första versionen. 
sidebar:
    order: 70
    hidden: true
---

import Figure from '@components/CustomFigure.astro';
import YouTube from '@components/YouTube.astro';

:::note
Detta dokument är enbart för lärare.
:::

Detta dokument beskriver hur en lärare kan skapa ett kursrepo likt det studenterna har, på enklaste möjliga sätt. Detta sätt kan även en student använda sig av, om man av någon anledning inte vill eller kan jobba igenom den guiden som visa hur man gör.



## Klona och duplicera med --mirror

Det finns ett [repo `algo-abcd25`](https://github.com/bth-algo/algo-abcd25) som används i guiden och det är skapat enligt guiden och det innnehåller också övrig utvecklingsmiljö som studenten sätter upp under kursens gång. Detta repo innehåller också samtliga utvecklingsverktyg som behövs innan man kan sätta igång med kmom01.

Börja med att ange det namn du vill ha på ditt repo.

```bash title="Ange namnet på ditt blivande repo."
# Set an environment variable with the repo name
REPO=algo-mirror
```

Nu kan du klona det exiterande repot och placera det i en katalog som matchar ditt önskade repo namn.

```bash title="Klona det befintliga repot och alla dess delar."
# Clone the existing repo as a mirror
git clone --mirror git@github.com:bth-algo/algo-abcd25.git $REPO
cd $REPO
```

Du behöver nu skapa ett nytt repo på GitHub som du vill koppla ditt lokala repo till.

När det är klart kan du koppla ditt lokala repo till den nyskapade platsen på GitHub.

```bash title="Koppla det lokala repot till platsen på GitHub."
# Connect your local repo with the place at GitHub
git remote set-url origin git@github.com:bth-algo/$REPO.git
git push --mirror
```

Nu är det klart och du kan börja jobba med ditt repo.

Här är direktlänken till det exempelrepo som är skapat som en mirror, [`algo-mirror`](https://github.com/bth-algo/algo-mirror).



## Forka ett exempelrepo

Man kan också skapa en fork av ett existerande repo, för att skapa en kopia av repot.

Här är en direktlänk till hur du [skapar en fork av exempelrepot `algo-abcd25`](https://github.com/bth-algo/algo-abcd25/fork).

Fyll i **Owner**, **Repository name** samt klicka ur checkboxen "Copy the main branch only".

import fork1 from '@assets/kursrepo/fork1.png';

<Figure 
    src={fork1}
    caption="Alla delar är ifyllda och nu kan du skapa din fork av exempelrepot."
/>

Här är direktlänken till det exempelrepo som är forkat, [`algo-fork`](https://github.com/bth-algo/algo-fork).
