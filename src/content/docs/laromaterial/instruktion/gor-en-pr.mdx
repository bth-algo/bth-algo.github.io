...
- `Tests passed(TP)`: Får din PR när de automatiska testerna gått igenom.
...
- `Grade: Ux`: En lärare har bedömt din inlämning men tycker att något behöver förbättras i din inlämning. Följ instruktionerna, fixa problemet lokalt och pusha din lösning till GitHug. När du taggar pushen så körs testerna om.
...
- mot `bth/submit/kmom03`

### kmom06

- mot `bth/submit/kmom06`

### kmom10 (Projektet)

- mot `bth/submit/kmom10`
...
Om något gick fel med testerna får du en kommentar om vad som gick fel samt din PR får labeln `Tests failed`. Eftersom du kör testerna när du taggar så bör detta inte inträffa.
...
1. Pusha din rättning till GitHub och gör en ny PR.
...
- Pusha din kod til GitHub. 
- Gör en ny tag, då körs testerna igen.
- Om din fix funkade ska du nu få labeln `Tests passed(TP)` eller `Tests failed`.

### Submission not approved / U

Om din inlämning inte är tillräckligt bra så får den labeln `Grade: U` och `U` i Canvas. Det kommer finnas kommentarer som förklara felet och läraren kommer stänga din PR.
